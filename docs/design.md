...
- `--review`（PR 作成 → Copilot レビュー → 修正の自動フロー）→ v1.4
- バナーバージョン表示 → v1.4
- `RunOptions` / `RunContext` struct 導入、モード別サブ関数分離 → v1.4
- `vibepod ps` / `vibepod logs` → v1.5
- `--mount`（追加ホストパスの read-only マウント）→ v1.5
- `vibepod.toml` 設定ファイル統合（config.json → config.toml）→ v1.5
- `--review` 進化（Codex + Copilot）→ v1.5
- bridge 削除・docker run 統一・run.rs 分割 → v1.5
