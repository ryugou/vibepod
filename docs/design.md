...
**非対話環境での挙動：** `vibepod login` は OAuth フローであり対話が本質的に必要なコマンドである。既存トークンが無い場合は確認プロンプトが無いため非対話環境（stderr が TTY でない）でも問題なく完走するが、既存トークンがあり（かつ未失効）上書き確認が必要な場合は、非対話環境では確認を取れないため自動で「はい」とはみなさず、エラーで中断する（`vibepod init` の agent 選択と異なり既定値へのフォールバックはしない）。対話端末から再実行するか、`vibepod logout` で既存トークンを先に削除してから再実行する。
