...
**失敗モード（非対話環境）：** 実行中の vibepod コンテナが存在するが非対話環境（stderr が TTY でない。CI・パイプ経由など）で実行された場合、削除確認を取れないため既存コンテナを削除せず非ゼロ終了する（他プロジェクトの実行中セッションを確認なしで巻き込まないため）。停止中コンテナのみの場合は非対話でも確認なしで削除される。
