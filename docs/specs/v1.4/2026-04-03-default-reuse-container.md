...
> **注意**: 本文書は v1.4 時点の設計スナップショットであり、現行仕様と乖離している箇所がある（例: `vibepod init` のコンテナ削除仕様）。現行仕様の正本は `docs/design.md` を参照すること。
...
> **廃止済み**: 以下の記述（running コンテナがある場合のみ確認、非インタラクティブ時は強制削除）は v1.4 時点のものであり、現在の実装とは異なる。現行仕様では、コンテナが 1 件でも存在すれば state を問わず対話環境で確認プロンプトを表示し、非対話環境では削除せずエラーで中断する。現行の正本は `docs/design.md` を参照すること。
>
