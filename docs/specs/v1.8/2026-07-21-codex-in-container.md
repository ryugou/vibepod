...
## codex レビュー指摘対応(round 8、2026-07-22)

round 7 対応は解消を確認。新規指摘2件、対応必須。

### P1: 非通常ファイル(ディレクトリ等)への差し替えが未処理(`src/cli/run/mod.rs:863-867` 付近)

symlink は削除するが、コンテナが `auth.json` をディレクトリ等の非通常ファイルに
置換した場合は素通りする。特に mtime が新しい `auth.json` ディレクトリは keep-newer 判定で
採用され、以後認証が恒久的に壊れる。

対応: ステージエントリの検査を「symlink なら削除」から「**通常ファイル(regular file)
以外はすべて削除して警告**」に一般化する(`symlink_metadata().file_type().is_file()` 判定。
ディレクトリは `remove_dir_all`)。警告文言は round 5 の tampering 系に合わせる。

### P2: mtime 同値時に更新済み認証がホスト側で上書きされる(`src/cli/run/mod.rs:889` 付近)

厳密な `>` 判定のため、ステージへのコピーとコンテナ内リフレッシュがファイルシステムの
タイムスタンプ分解能内(粗い fs では同一秒)に起きると mtime が同値になり、
更新済みステージが古いホスト認証で上書きされる。

対応:
- 判定を「**同値はステージ優先**」(`staged >= host` で保持)に変更する。
  同値でかつ実際にはホストが新しい(同一瞬間の再ログイン)ケースは天文学的に稀で、
  再ログインし直せば回復するため、リフレッシュトークン喪失(回復にユーザー操作必須)より
  安全側に倒す。この根拠をコードコメントに書くこと
- あわせて、コピー前に**内容が同一ならコピー自体を省略**する(バイト比較。ファイルは
  数 KB なので全読みでよい)。無駄な mtime 更新と rename を避け、判定の安定性も上がる

### テスト追加(round 8 対応分)

- ステージの auth.json がディレクトリに差し替えられている → 削除され、ホストから再作成される
