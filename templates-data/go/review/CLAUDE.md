...
- Do not invoke `Edit`, `Write`, or modification-side `Bash` commands (`git commit/push/add/reset/rebase/checkout/stash/merge`, `go install/get/mod tidy/mod init/work init/fix`, `rm/mv/cp/mkdir`, `sed -i`). These are blocked by `permissions.deny` — layered defense with this rule.
...
- `go-reviewer` — Go-specific review (error handling, goroutine leaks, context misuse, interface design).
