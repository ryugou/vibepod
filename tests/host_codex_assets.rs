...
use vibepod::cli::run::{
    finalize_disposable_runtime_dir, host_codex_stage_entries, prepare_codex_mount,
    should_keep_staged_auth, sync_codex_stage_to_store, ContainerLiveness,
};
...
// --- sync_codex_stage_to_store: run-after JSON-integrity gate
// (codex review round 11 P1, made unconditional in round 12 P1-a) ---
//
// The rw-mounted stage auth.json is untrusted container-writable input. It can
// be observed mid-write while the container keeps running, and it can also be
// left truncated when `docker stop` escalates to SIGKILL — both produce invalid
// JSON. Ingesting either would persist a corrupted token into the store, which
// is distributed to every container. So the ingest is gated on a successful
// `serde_json` parse **unconditionally** (round 12 P1-a: no longer keyed on
// container liveness), leaving the store untouched on failure.

#[test]
fn sync_codex_stage_to_store_skips_invalid_json_stage_and_keeps_store() {
    let home_dir = tempfile::tempdir().unwrap();
    let config_dir = tempfile::tempdir().unwrap();
    let runtime_dir = tempfile::tempdir().unwrap();
    make_host_codex(home_dir.path());
    ...
    assert_eq!(
        ...
        "a mid-write (invalid JSON) stage auth.json must never overwrite the store"
    );
...
#[test]
fn sync_codex_stage_to_store_skips_invalid_json_stage_and_leaves_empty_store() {
...
#[test]
fn sync_codex_stage_to_store_promotes_valid_json_stage() {
    // Valid JSON passes the integrity gate, so the normal keep-newest
    ...
    sync_codex_stage_to_store(runtime_dir.path(), config_dir.path())
        .expect("a well-formed, newer stage auth.json must sync successfully");

    assert_eq!(
        fs::read_to_string(&store_auth).unwrap(),
        r#"{"token":"CONTAINER_REFRESHED_VALID"}"#,
        "a valid, newer stage auth.json must be promoted into the store"
    );
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
...
#[test]
fn sync_codex_stage_to_store_skips_invalid_json_stage_unconditionally() {
    // round 12 P1-a reversal: previously a `Stopped` sync deliberately skipped
    // the JSON-integrity gate and copied invalid bytes through verbatim, on the
    // assumption that a stopped container could not leave a torn write. But
    // `docker stop` escalating to SIGKILL can leave a truncated auth.json even
    // on a stopped container, so the gate is now unconditional: the very same
    // truncated payload that used to be copied under `Stopped` must now be
    // rejected, leaving the store completely untouched.
    let home_dir = tempfile::tempdir().unwrap();
    let config_dir = tempfile::tempdir().unwrap();
    let runtime_dir = tempfile::tempdir().unwrap();
    make_host_codex(home_dir.path());
    ...
    // The store already holds a valid auth.json (host -> store hop). Pin its
    // content and mtime so we can assert neither changes.
    let store_content_before = fs::read_to_string(&store_auth).unwrap();
    let t0 = SystemTime::now();
    set_mtime(&store_auth, t0);
    let store_mtime_before = fs::metadata(&store_auth).unwrap().modified().unwrap();

    // A truncated (mid-write / SIGKILL-interrupted) payload with a strictly
    // newer mtime that keep-newest would otherwise promote.
    fs::write(&staged_auth, r#"{"token":"NOT_VALID_JSON_TRUNCATED"#).unwrap();
    set_mtime(&staged_auth, t0 + Duration::from_secs(3_600));

    sync_codex_stage_to_store(runtime_dir.path(), config_dir.path())
        .expect("sync must not fail the run's cleanup just because the snapshot was truncated");

    assert_eq!(
        fs::read_to_string(&store_auth).unwrap(),
        store_content_before,
        "an invalid-JSON stage must never overwrite the store, regardless of container liveness"
    );
    let store_mtime_after = fs::metadata(&store_auth).unwrap().modified().unwrap();
    assert_eq!(
        store_mtime_before, store_mtime_after,
...
// --- finalize_disposable_runtime_dir: docker cleanup success/failure decides
// whether the bind-mounted runtime dir (containing the codex stage) may be
// deleted (codex review round 12 P1-b) ---
//
// The disposable cleanup path used to drop `docker rm -f`'s result with `.ok()`
// and always delete the runtime dir. If removal actually failed (daemon down,
// permissions), the container could still be running with the stage bind-mounted
// rw — deleting it out from under a live container is unsafe. `finalize_
// disposable_runtime_dir` takes the liveness the caller derived from the exit
// status: `Stopped` (removal confirmed) deletes the runtime dir; `Running`
// (removal failed / container may survive) syncs but preserves it for the next
// run. Either way the codex stage is synced through the same integrity-checked
// path first.

#[test]
fn finalize_disposable_runtime_dir_preserves_runtime_dir_when_removal_failed() {
    // Failure path (Running): the runtime dir must survive, and the codex stage
    // must still be synced (integrity-checked) into the store so a refreshed
    // token is not silently lost while we wait for manual cleanup.
    let home_dir = tempfile::tempdir().unwrap();
    let config_dir = tempfile::tempdir().unwrap();
    let runtime_dir = tempfile::tempdir().unwrap();
    make_host_codex(home_dir.path());
    ...
    // A well-formed, strictly newer stage token that keep-newest must promote.
    let t0 = SystemTime::now();
    set_mtime(&store_auth, t0);
    fs::write(
        &staged_auth,
        r#"{"token":"REFRESHED_BEFORE_FAILED_REMOVAL"}"#,
    )
    .unwrap();
    set_mtime(&staged_auth, t0 + Duration::from_secs(3_600));

    finalize_disposable_runtime_dir(
        runtime_dir.path(),
        config_dir.path(),
        true,
    ...
    assert!(
        runtime_dir.path().exists(),
        "a failed container removal (Running) must preserve the runtime dir, not delete the \
         bind-mounted stage under a possibly-live container"
    );
    assert_eq!(
        fs::read_to_string(&store_auth).unwrap(),
        r#"{"token":"REFRESHED_BEFORE_FAILED_REMOVAL"}"#,
        "the codex stage must still be synced into the store even when the runtime dir is kept"
    );
}

#[test]
fn finalize_disposable_runtime_dir_deletes_runtime_dir_when_removal_succeeded() {
    // Success path (Stopped): removal confirmed, so the runtime dir (including
    // the stage) is deleted after the stage has been synced into the store.
    let home_dir = tempfile::tempdir().unwrap();
    let config_dir = tempfile::tempdir().unwrap();
    let runtime_dir = tempfile::tempdir().unwrap();
    make_host_codex(home_dir.path());
    ...
    fs::write(
        &staged_auth,
        r#"{"token":"REFRESHED_BEFORE_CLEAN_REMOVAL"}"#,
    )
    .unwrap();
    set_mtime(&staged_auth, t0 + Duration::from_secs(3_600));
    ...
    assert!(
        !runtime_dir.path().exists(),
        "a confirmed container removal (Stopped) must delete the runtime dir"
    );
    // The store keeps the token that was synced before the runtime dir was
    // deleted (sync happens first inside finalize).
    assert_eq!(
        fs::read_to_string(&store_auth).unwrap(),
        r#"{"token":"REFRESHED_BEFORE_CLEAN_REMOVAL"}"#,
        "the stage must be synced into the store before the runtime dir is deleted"
    );
}
