...
2. **Minimal mounts** — only what the agent needs is mounted:
   ...
   - `~/.claude/plugins/` (read-only, when present): your installed Claude Code plugins — mounted at both `/home/vibepod/.claude/plugins` and the host absolute path to resolve `installed_plugins.json` entries. The `plugins/data/` subdirectory is the one exception: it is replaced by a per-container **empty, writable** stage (`~/.config/vibepod/runtime/<container>/plugins-data/`), layered inside the read-only `plugins/` mount. Host `plugins/data/` content is never copied in, and anything a plugin writes there inside the container never propagates back to the host — this is what lets plugins that need to write job/state data there (e.g. the codex plugin) work at all, without carrying other projects' plugin data into the container. The stage is only empty at creation time: recreating the container with `--new` does **not** clear it, so anything written on a previous run (e.g. codex job history) carries over; only a disposable `--worktree` run deletes it, along with the rest of that run's runtime directory, when it exits
