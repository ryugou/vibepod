...
2. **Minimal mounts** — only what the agent needs is mounted:
   ...
   - `~/.claude/plugins/` (read-only, when present): your installed Claude Code plugins — mounted at both `/home/vibepod/.claude/plugins` and the host absolute path to resolve `installed_plugins.json` entries. The `plugins/data/` subdirectory is the one exception: it is replaced by a per-container **writable** stage (`~/.config/vibepod/runtime/<container>/plugins-data/`), layered inside the read-only `plugins/` mount. Host `plugins/data/` content is never copied in, and anything a plugin writes there inside the container never propagates back to the host — this is what lets plugins that need to write job/state data there (e.g. the codex plugin) work at all, without carrying other projects' plugin data into the container. The stage starts **empty every time a container is newly created** — this holds regardless of path (first run, after `--new`, or after `vibepod rm`); a run that reuses an existing container instead keeps whatever the stage already holds (e.g. codex job history from earlier in that container's life), so an in-progress plugin job isn't wiped out from under it. A disposable `--worktree` run deletes the stage, along with the rest of that run's runtime directory, when it exits
